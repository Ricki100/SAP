-- SAPC CMS schema. Run once in the SAPC Supabase SQL Editor.
...
create table if not exists public.blog_brand_settings (
  ...
  brand_name text not null default 'SAPC',
  ...
  light_link text not null default '#004d62',
  dark_background text not null default '#004d62',
  dark_text text not null default '#ffffff',
  dark_link text not null default '#f99d1c',
  updated_at timestamptz not null default now(),
...
insert into public.blog_brand_settings (id) values ('default')
on conflict (id) do update set
  brand_name = 'SAPC',
  theme_mode = 'light',
